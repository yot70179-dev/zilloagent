...
### תבניות פולואפ (למי שלא ענה — גם הן צריכות אישור)
צור עוד שתי תבניות Marketing (אותו תהליך, עם `{{1}}`=שם, `{{2}}`=עסק):

**`landing_page_followup1`** (נשלח אחרי 3 ימים):
```
היי {{1}}, רק מוודא שההודעה הקודמת הגיעה 🙂 אשמח לשלוח לך את דוגמת דף הנחיתה שהכנתי ל{{2}} — בלי שום התחייבות. שאשלח?
```
**`landing_page_followup2`** (נשלח אחרי 7 ימים):
```
היי {{1}}, לא רוצה להציק — אם זה לא רלוונטי כרגע, לגמרי בסדר. הדוגמה עדיין מוכנה אם תרצה/י להציץ. בהצלחה עם {{2}}! 🙏
```
כששתיהן מאושרות → `WHATSAPP_TEMPLATE_FOLLOWUP1=landing_page_followup1` ו-`WHATSAPP_TEMPLATE_FOLLOWUP2=landing_page_followup2`.
> אם לא תגדיר תבניות פולואפ — המערכת עדיין תנסה לשלוח פולואפ כטקסט חופשי, אבל
> Meta תחסום אותו למי שלא ענה ב-24 שעות. לכן שווה לאשר גם את השתיים האלה.
